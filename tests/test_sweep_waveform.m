classdef test_sweep_waveform < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function command_is_a_deviation_not_an_absolute_level(tc)
            % The amplifier owns holding. If software also emitted the holding
            % level, a cell the Commander holds at -70 mV would sit at -140.
            % So the command rests at 0 and everything rides on the front panel.
            fs = 20000;
            cfg = tc.sweepCfg();
            cmd = sem.protocol.sweepWaveform(cfg, 'VC', fs);

            tc.verifyEqual(cmd(1), 0, 'AbsTol', 1e-9, ...
                'the sweep must open at holding, i.e. zero deviation');
            tc.verifyEqual(cmd(end), 0, 'AbsTol', 1e-9, ...
                'and return to it, since the AO idles at its last sample');
            % The test pulse is the only excursion, and it is the configured one.
            tc.verifyEqual(min(cmd), -5, 'AbsTol', 1e-9);
        end

        function command_train_rides_on_holding(tc)
            % A +20 mV step is +20 FROM the front-panel level, whatever it is.
            fs = 20000;
            cfg = tc.sweepCfg();
            cfg.command = struct('startTimeMs', 300, 'nPulses', 1, ...
                'pulseDurationMs', 50, 'amplitude', 20, 'frequencyHz', 1);
            cmd = sem.protocol.sweepWaveform(cfg, 'VC', fs);
            tc.verifyEqual(max(cmd), 20, 'AbsTol', 1e-9);
        end
