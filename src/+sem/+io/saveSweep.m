...
meta.ephys = struct( ...
    ...
    'kind',             'sweep', ...
    'clampMode',        modeC, ...
    'holdingSource',    'amplifier', ...
    'holdingCommandMv', holdingCommand(sweep), ...
    'gains',            gains, ...
...
function h = holdingCommand(sweep)
%holdingCommand What the amplifier was holding at, or NaN if unknown.
%   The Commander owns holding; this records what it reported rather than
%   what software commanded, because software commands nothing here.
if isfield(sweep, 'holdingCommandMv')
    h = sweep.holdingCommandMv;
else
    h = NaN;
end
end
